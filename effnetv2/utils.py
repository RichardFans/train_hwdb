...
def get_ckpt_var_map(ckpt_path,
    ...
    for v in model_vars:
        ...
        flist = ['global_step', 'ExponentialMovingAverage',
                 'Momentum', 'RMSProp']
        if list(filter(lambda x, s=v_name: x in s, flist)):
            continue

        if not v.op.name.startswith(var_scope):
            logging.info('skip %s -- does not match scope %s',
                         v_name, var_scope)
        ...
        if v.shape != cv_shape:
            if skip_mismatch:
                ...
                continue
            raise ValueError(
                f'shape mismatch {v_name} ({v.shape} vs {cv_shape})')

        var_map[cv_name] = v

    if not var_map or len(var_map) < 5:
        raise ValueError(
            f'var_map={var_map} is almost empty, please check logs.')

    for (k, v) in var_map.items():
        logging.log_first_n(
            logging.INFO, f'Init {v.op.name} from ckpt var {k}', 10)
...
def restore_tf2_ckpt(model,
    ...
    if var_list[0][0] == '_CHECKPOINTABLE_OBJECT_GRAPH':
        print(
            f'Load checkpointable from {ckpt_file}, excluding {exclude_layers}')
