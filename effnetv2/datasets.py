...
import effnetv2.preprocessing as preprocessing
...
class ImageNetInput():
    ...
    cfg = hparams.Config(
        ...
        splits=dict(
            ...
            minival=dict(num_images=25_021, files='train*',
                         slice=slice(0, 20)),
    ...
    def make_source_dataset(self, index, num_hosts):
        ...
        if self.is_training and self.shuffle_files:
            ...
            dataset = dataset.shuffle(
                num_files_per_shard, seed=self.shuffle_seed)
...
class HwdbCasiaInput():
    cfg = hparams.Config(
        data_dir=None,
        num_classes=6825,
        multiclass=False,
        splits=dict(
            train=dict(num_images=1_535_625),
            eval=dict(num_images=170_625),
        ))
...
class CIFAR10Input(ImageNetInput):
    ...
    def preprocess(self, features):
        ...
        new_label = {'label': tf.one_hot(
            features['label'], self.cfg.num_classes)}
    ...
    def _input_fn(self, batch_size, current_host, num_hosts):
        ...
        if self.is_training:
            if self.cache:
                ...
                ds = ds.shuffle(self.shuffle_size_k * 1024,
    ...
    cfg.update(
        dict(
            ...
            splits=dict(
                ...
                minival=dict(num_images=1020,
                             tfds_split='validation', slice=''),
...
@ds_register
class HwdbCasia(ImageNet21k):
    """Hwdb Casia 1.0 and 1.2 configs."""
    cfg = copy.deepcopy(ImageNet21k.cfg)
    cfg.data.ds_name = 'hwdbcasia'
